...
skeleton <- function(suffStat, indepTest, alpha, labels, p,
		     ...
		     NAdelete = TRUE, numCores = 1, verbose = FALSE,entropic=FALSE)
{
  ...
  if (method == "stable.fast") {
    ...
    ## Original R version
    # MURAT HERE. ONLY MODIFIED THE R VERSION.
    if (entropic==FALSE){}
    else {
      entropies_df=entropic[1]
      pairwise_entropies_df=entropic[2]
      thresholds_df=entropic[3]
    }
    print(entropies_df)
    pval <- NULL
        ...
        if (G[y, x] && !fixedEdges[y, x]) {
          ...
          if (length_nbrs >= ord) {
            ...
            repeat { ## condition w.r.to all  nbrs[S] of size 'ord'
              ...
              if (pMax[x, y] < pval)
                pMax[x, y] <- pval
              if(pval >= alpha) { # independent MURAT: ADD AN AND STATEMENT HERE USING ENTROPY OF SEP SET AND COMMON ENTROPY
                G[x, y] <- G[y, x] <- FALSE
                sepset[[x]][[y]] <- nbrs[S]
                break
              }
...
pc <- function(suffStat, indepTest, alpha, labels, p,
               ...
               solve.confl = FALSE, numCores = 1, verbose = FALSE, entropic=FALSE)
{
  ## Purpose: Perform PC-Algorithm, i.e., estimate skeleton of DAG given data
  ## ----------------------------------------------------------------------
  ## Arguments:
  ...
  if (entropic==FALSE) {
    ...
    skel <- skeleton(suffStat, indepTest, alpha, labels = labels, method = skel.method,
                     fixedGaps = fixedGaps, fixedEdges = fixedEdges,
                     NAdelete=NAdelete, m.max=m.max, numCores=numCores, verbose=verbose,entropic=entropic)}
